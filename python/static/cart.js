...
let generateCartItems = () => {
  if (basket.length !== 0) {
    ...
    <h2>Cart is Empty</h2>
    <a >
      <button class="HomeBtn" onclick="homebtn()">Back to Home</button>
    </a>
    `;
  }
};
...
let TotalAmount = () => {
  if (basket.length !== 0) {
    ...
    <button class="checkout" onclick="document.getElementById('id01').style.display='block'" style="width:auto;">Checkout <i class="bi bi-bag-check-fill"></i></button>
    <button onclick="clearCart()" class="removeAll">Clear Cart <i class="bi bi-trash"></i></button>
    `);
  } else return;
};
